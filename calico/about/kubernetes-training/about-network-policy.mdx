...
To learn more about $[prodname] network policies, see [Calico network policy](network-policy/get-started/calico-policy/calico-network-policy.mdx) guide.
...
![Example mix of network policy types](/img/calico-enterprise/example-k8s-calico-policy-mix.svg)
...
spec:
  namespaceSelector: has(kubernetes.io/metadata.name) && kubernetes.io/metadata.name not in {"kube-system", "calico-system", "calico-apiserver"}
  types:
    - Ingress
    - Egress
  egress:
...
![Example tiers](/img/calico-enterprise/example-tiers.svg)
